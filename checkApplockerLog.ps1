...
Get-WinEvent -FilterHashtable @{logname="Microsoft-Windows-Applocker/*"; id=8004} -ComputerName $system | select Message -unique | Out-File -append temp.txt #-ComputerName WVDES230002 
...
Get-WinEvent -FilterHashtable @{logname="Microsoft-Windows-Applocker/*"; id=8003} -ComputerName $system| select Message -unique | Out-File -append temp.txt #-ComputerName WVDES230002 
